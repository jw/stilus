...
def test_cli_out_and_print():
    ...
    with runner.isolated_filesystem() as fs:
        ...
        result = runner.invoke(
            cli.stilus, ["--out", "some_directory", "foo.styl"]
        )
        css = "foo {\n  abc: #00f;\n}\n"
        ...
        result = runner.invoke(
            cli.stilus, ["--out", "some_directory", "foo.styl", "foo.css"]
        )
        css_file = Path(fs) / Path("some_directory") / Path("foo.css")
        ...
        result = runner.invoke(
            cli.stilus,
            ["--out", "some_directory", "-p", "foo.styl", "foo.css"],
        )
        css_file = Path(fs) / Path("some_directory") / Path("foo.css")
        assert css_file.exists() is True
        assert css_file.read_text() == css
...
def test_prepare_watch():
    with TemporaryDirectory() as td:
        ...
        assert sorted(styles) == sorted([path / "foo.styl", path / "bar.styl"])
        assert sorted(list(path.glob("*.css"))) == sorted(
            [path / "foo.css", path / "bar.css"]
        )
